...
static void start_process(void *_args)
{
  ...
  // Remove the initialization of parent's children_list here
  // It should only be initialized once when the parent thread is created
  struct thread *cur = thread_current ();
  ...
  // Add the child to parent's list (which should already be initialized)
